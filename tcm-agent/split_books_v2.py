...
def split_huxishuo(book="伤寒论释义-胡希恕"):
    """伤寒论释义-胡希恕：按第X章、第X部分切分"""
    fname = os.path.join(SRC, BOOKS[book])
    with open(fname, encoding="utf-8") as f:
        text = f.read()
    
    # 优先按"第X部分"切分，其次按"第X章"切分
    # 先尝试找第X部分
    pat_section = re.compile(r'^(第[一二三四五六七八九十百零0-9０-９]+部分)', re.MULTILINE)
    pat_chapter = re.compile(r'^(第[一二三四五六七八九十百零0-9０-９]+章)', re.MULTILINE)
    
    # 先用部分分割
    parts = pat_section.split(text)
    if len(parts) > 2:
        current_chapter = "未分类"
        current_content = ""
        for i, part in enumerate(parts):
...
def split_huangdi_v2(book="黄帝内经"):
    """黄帝内经：按篇名切分（素问/灵枢各篇）"""
    fname = os.path.join(SRC, BOOKS[book])
    with open(fname, encoding="utf-8") as f:
        text = f.read()
    ...
    # 找"篇名"或"第X篇"模式
    pat = re.compile(r'^《*(素问|灵枢)[》]*[：:\s]*[第]?[卷篇]?[第]?([一二三四五六七八九十百零0-9０-９]+)[篇章部：:\s]', re.MULTILINE)
    pat2 = re.compile(r'^第([一二三四五六七八九十百零0-9０-９]+)[篇章]', re.MULTILINE)
    pat3 = re.compile(r'^《?(素问|灵枢|阴阳应象大论|六节藏象论|五脏生成论|五脏别论|经脉别论|脏气法时论|宣明五气篇|血气形态篇|病因病机论|标本病传论|天年论|至真要大论)[篇]?[》]?', re.MULTILINE)
    ...
    if len(parts) > 2:
        ...
        split_by_pattern(book, r'^第([一二三四五六七八九十百零0-9０-９]+)[篇章]', "篇切分", chunk_target=1000)
