...
"""
盯盘小助手 v3 — 后台自动轮询
用openclaw CLI发消息到飞书群（已配置account=main）
...
FEISHU_TARGET = "chat:oc_8eb8a6d7127a1828331123cee9b3eb92"
FEISHU_CHANNEL = "feishu"
FEISHU_ACCOUNT = "main"  # 实际配置的账号名
...
STOP_LOSS_PCT = -8.0  # 浮亏超过8%提醒

# 状态追踪
last_prices = {}
alert_cooldown = {}
COOLDOWN = 120  # 同一提醒2分钟冷却
...
def send_alert(msg):
    try:
        ...
        if result.returncode != 0:
            print(f"[ERROR] 推送失败: {result.stderr[:300]}")
        else:
            print(f"[SENT] {msg[:50]}...")
...
def check_and_alert(code, quote, pos):
    ...
    if price < low:
        ...
        if key not in alert_cooldown or (now - alert_cooldown[key]) > COOLDOWN:
            alerts.append(f"🚨 {name} 跌破{low}防线！当前{price:.2f}，浮亏{pnl_pct:.1f}%，建议减仓")
    ...
    if price > high:
        key = f"{code}_high"
        if key not in alert_cooldown or (now - alert_cooldown[key]) > COOLDOWN:
            alerts.append(f"📤 {name} 冲到{high}以上！当前{price:.2f}，建议高抛")
            alert_cooldown[key] = now
    ...
    if pnl_pct < STOP_LOSS_PCT:
        ...
        if key not in alert_cooldown or (now - alert_cooldown[key]) > 600:
            alerts.append(f"🔴 {name} 浮亏已达{pnl_pct:.1f}%（成本{cost}，现价{price:.2f}），建议止损")
            alert_cooldown[key] = now
    ...
    if code in last_prices and last_prices[code] > 0:
        ...
        if drop < -1.5:
            ...
            if key not in alert_cooldown or (now - alert_cooldown[key]) > 180:
                alerts.append(f"⚡ {name} 急跌{drop:.1f}%（{last_prices[code]:.2f}→{price:.2f}），注意！")
                alert_cooldown[key] = now
    ...
    if 30 <= market_minutes <= 37:
        ...
        if yclose > 0 and price > yclose * 1.01:
            ...
            if key not in alert_cooldown:
                alerts.append(f"🕐 {name} 开盘冲{price:.2f}（>昨收+1%），高抛窗口！建议减仓")
                alert_cooldown[key] = now
...
def main():
    ...
    print(f"📢 推送到飞书群(账号:{FEISHU_ACCOUNT})")
    print("=" * 50)
    ...
    while True:
        ...
        if raw:
            ...
            if all_alerts:
                ...
                msg = f"📊 [{now_str}] 大盘: {sh_str} | 深: {sz_str}\n"
                for a in all_alerts:
                    msg += f"{a}\n"
                print(f"\n[{now_str}] 🚨 触发预警!")
                print(msg)
                send_alert(msg.strip())
            elif cycle % 40 == 0:  # 每10分钟打印心跳
                status = []
                for code, pos in POSITIONS.items():
                    q = parse(raw, code)
                    if q:
