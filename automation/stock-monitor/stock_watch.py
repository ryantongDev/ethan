...
THRESHOLDS = {
    "000001": {
        ...
        "pos": "大盘监控",
        "watch": [
            (4050, 4080, "回落提醒①", "回落至4050~4080，准备低吸"),
            (4080, 4120, "接近压力", "接近4120，冲不过要卖"),
        ],
    },
    "300001": {
        "name": "特锐德",
        "pos": "300股@30.82（储能龙头）",
        "sell": [
            (31.50, 32.50, "T出提醒", "目标价，止盈"),
        ],
        "buy": [
            (29.00, 29.50, "⚠️快到买区", "提前预警：准备下单，止损27.50"),
            (29.10, 29.30, "✅买入确认", "5日线支撑，可以买"),
        ],
        "stop": 27.50,
        "stop_label": "止损警戒",
        "stop_msg":  "⚠️ 跌破止损线！",
    ...
    "000049": {
        ...
        "pos": "300股@29.39（超短）",
        "sell": [
            (30.00, 30.50, "T出提醒", "超短目标，止盈"),
        ],
        "buy": [
            (28.50, 28.80, "⚠️快到买区", "提前预警：准备下单"),
            (28.50, 29.00, "✅买入确认", "超短买点，止损28.00"),
        ],
        "stop": 28.00,
        "stop_label": "止损警戒",
        "stop_msg":  "⚠️ 跌破止损线！",
    ...
    "300903": {
        ...
        "pos": "关注（PCB方向）",
        "sell": [
            (56.00, 58.00, "T出提醒", "高位止盈"),
        ],
        "buy": [
            (53.00, 54.50, "⚠️快到买区", "提前预警：等回踩再买"),
            (53.00, 54.00, "✅买入确认", "回踩买区，可建仓"),
        ],
        "stop": 52.00,
        "stop_label": "止损警戒",
        "stop_msg":  "⚠️ 跌破止损线！",
    ...
    "300687": {
        ...
        "pos": "关注（华为软件）",
        "sell": [
            (28.50, 29.50, "T出提醒", "高位止盈"),
        ],
        "buy": [
            (26.00, 26.80, "⚠️快到买区", "提前预警：等回踩再买"),
            (26.00, 26.50, "✅买入确认", "回踩买区，可建仓"),
        ],
        "stop": 25.50,
        "stop_label": "止损警戒",
        "stop_msg":  "⚠️ 跌破止损线！",
    ...
    "603650": {
        ...
        "pos": "关注（PCB方向）",
        "sell": [
            (58.00, 60.00, "T出提醒", "高位止盈"),
        ],
        "buy": [
            (54.00, 55.50, "⚠️快到买区", "提前预警：等回踩再买"),
            (54.00, 55.00, "✅买入确认", "回踩买区，可建仓"),
        ],
        "stop": 53.00,
        "stop_label": "止损警戒",
        "stop_msg":  "⚠️ 跌破止损线！",
    ...
    "300545": {
        ...
        "pos": "关注（先进制造）",
        "sell": [
            (36.00, 38.00, "T出提醒", "高位止盈"),
        ],
        "buy": [
            (33.50, 34.50, "⚠️快到买区", "提前预警：等回踩再买"),
            (33.50, 34.00, "✅买入确认", "回踩买区，可建仓"),
        ],
        "stop": 32.50,
        "stop_label": "止损警戒",
        "stop_msg":  "⚠️ 跌破止损线！",
    ...
    "516160": {
        ...
        "buy": [
            (3.20, 3.30, "⚠️快到买区", "提前预警：等回踩再买"),
            (3.20, 3.28, "✅买入确认", "回踩买区，可建仓"),
        ],
        "stop": 3.10,
        "stop_label": "止损警戒",
        "stop_msg":  "⚠️ ETF跌破止损线！",
    ...
    "588000": {
        ...
        "pos": "5000股@1.744",
        "sell": [
            (1.76, 1.82, "T出提醒", "+5%以上，锁定利润"),
        ],
        "buy": [
            (1.70, 1.74, "⚠️快到买区", "提前预警：可以加仓"),
            (1.70, 1.73, "✅买入确认", "回踩支撑，可加仓"),
        ],
        "stop": 1.65,
        "stop_label": "止损警戒",
        "stop_msg":  "⚠️ ETF跌破止损线！",
...
def check_thresholds(data: dict, old: dict | None) -> list[dict]:
    ...
    # ── Watch (大盘) signals ─────────────────────────────────
    prev_sent_w = (old or {}).get("_sent_watch", [])
    for (lo, hi, label, reason) in cfg.get("watch", []):
        if lo <= price <= hi:
            if label not in prev_sent_w:
                alerts.append({
                    "type":      "watch_alert",
                    "symbol":    sym,
                    "name":      cfg["name"],
    ...
    if mode == "once":
        ...
        for data in fetch_quotes():
            ...
            if thresh_alerts:
                ...
                new_state[sym]["_sent_watch"] = [a["label"] for a in thresh_alerts if a["type"] == "watch_alert"]
            else:
                new_state[sym]["_sent_sell"] = old_state.get(sym, {}).get("_sent_sell", [])
                new_state[sym]["_sent_buy"]  = old_state.get(sym, {}).get("_sent_buy", [])
                new_state[sym]["_sent_watch"] = old_state.get(sym, {}).get("_sent_watch", [])
